...
export function getQuestType(task) {
  ...
  const combinedDesc = ((task.introduction || "") + " " + (task.instructions || "") + " " + (task.submissionGuidelines || "")).toLowerCase();

  if (
    title.includes("[main]") ||
    title.includes("main quest") ||
    title.includes("utama") ||
    combinedDesc.includes("main quest") ||
    combinedDesc.includes("misi utama")
  ) {
    return "MAIN";
  }
...
function QuestCard({ task, submission, onOpen, index, isLocked = false }) {
  ...
  return (
    <motion.div
      ...
      className={`group relative bg-white dark:bg-[#08120e] border rounded-xl p-4 sm:p-5.5 transition-all duration-300 cursor-pointer hover:scale-[1.008] overflow-hidden w-full max-w-full min-w-0 ${
        isLocked
    ...
    >
      ...
      <div className="flex items-start gap-3 sm:gap-4 relative z-10 w-full min-w-0">
        {/* Quest Icon Badge */}
        <div
          className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0 transition-transform duration-300 border shadow-sm ${
            isLocked
        ...
        <div className="flex-1 min-w-0 pr-1 sm:pr-4">
          ...
          <div className="flex items-center justify-between gap-1.5 mb-1.5 w-full flex-wrap min-w-0">
...
function QuestDetailModal({ task, submission, onClose, onSubmitSuccess, isLocked = false }) {
  ...
  // Lock body & document scroll when modal is active
  useEffect(() => {
    ...
    >
      ...
      >
        ...
        <div className="flex items-start justify-between p-4 sm:p-6 pb-3 sm:pb-4 bg-white dark:bg-[#07130e] border-b border-slate-100 dark:border-white/5 shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-2 sm:p-2.5 rounded-lg bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-400 dark:text-white/40 transition-colors cursor-pointer shrink-0"
          >
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-5 overflow-y-auto flex-1 overscroll-contain">
          ...
          {isLateSubmissionBlocked && status !== "APPROVED" && (
            <div className="p-5 bg-rose-500/10 border border-rose-500/25 rounded-xl flex items-start gap-3.5 shadow-sm">
          ...
          {/* 3 Structured Quest Content Sections */}
          <div className="space-y-4">
            {/* 1. Pendahuluan */}
            {task.introduction && (
              <div className="bg-slate-50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 rounded-xl p-4 sm:p-5 space-y-2">
            ...
            {/* 2. Tugas / Pertanyaan (Main Quest) or Arahan Pengerjaan (Side Quest) */}
            {task.instructions && (
              <div className="bg-slate-50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 rounded-xl p-4 sm:p-5 space-y-2">
                <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
                  <Target className="w-3.5 h-3.5 text-emerald-500" />
                  {!isMain
                    ? (language === "en" ? "2. Side Quest Instructions" : "2. Arahan Pengerjaan")
            ...
            {task.submissionGuidelines && (
              <div className="bg-amber-500/5 dark:bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 sm:p-5 space-y-2">
            ...
            <>
              ...
              {(status === "PENDING" || status === "APPROVED") && (
                submission?.score !== null && submission?.score !== undefined ? (
                  <div className="p-4 bg-emerald-500/10 border border-emerald-500/25 rounded-xl space-y-2.5">
              ...
              {canSubmit && task.submissionType !== "TTS" && task.submissionType !== "FORM" && (
                <form onSubmit={handleSubmit} className="space-y-4 pt-2">
                  ...
                  {type === "link" && (
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-11 pr-4 py-3.5 bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/10 rounded-xl text-sm font-medium text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-white/25 focus:outline-none focus:border-emerald-500/50 focus:ring-2 focus:ring-emerald-500/10 transition-all"
                      />
                    </div>
                  )}
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <button
                          ...
                          className="mt-2 px-4 py-2 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-black transition-colors cursor-pointer"
                        >
                      ...
                      {files.length > 0 && (
                        <div className="mt-4 space-y-2 text-left">
                          {files.map((file, idx) => (
                            <div
                              key={idx}
                              className="flex items-center justify-between p-2.5 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg"
                            >
                  ...
                  {error && (
                    <div className="flex items-center gap-2 p-3.5 bg-rose-500/10 border border-rose-500/20 text-rose-500 text-xs font-bold rounded-lg">
                      <AlertTriangle className="w-4 h-4 flex-shrink-0" />
                      <span>{error}</span>
                    </div>
                  )}
                  {success && (
                    <div className="flex items-center gap-2 p-3.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-xs font-bold rounded-lg">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full py-3.5 bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-black text-xs uppercase tracking-widest rounded-xl shadow-md hover:scale-[1.01] transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                  >
                    {loading ? (
                      <>
  ...
  return (
    <div className="w-full max-w-full space-y-6 sm:space-y-8 select-none overflow-x-hidden">
      ...
      >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 md:flex items-center gap-2 sm:gap-2.5 w-full md:w-auto">
          {[
            ...
            <div
              key={s.label}
              className="flex flex-col items-center justify-center px-3 sm:px-3.5 py-2 bg-white dark:bg-[#08120e] border border-slate-200/80 dark:border-white/5 rounded-xl shadow-sm min-w-0"
            >
      ...
      <motion.div
        ...
        className="w-full max-w-full overflow-x-auto no-scrollbar min-w-0"
      >
        <div className="flex items-center gap-2 p-1.5 bg-slate-100 dark:bg-white/5 border border-slate-200/80 dark:border-white/8 rounded-xl w-max min-w-0">
          <button
            onClick={() => setQuestCategory("ALL")}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-xs font-black whitespace-nowrap transition-all cursor-pointer ${
              questCategory === "ALL"
      ...
      <motion.div
        ...
        className="flex flex-col lg:flex-row gap-3 w-full max-w-full min-w-0"
      >
        {/* Status Filter tabs */}
        <div className="w-full max-w-full min-w-0 lg:w-auto overflow-x-auto no-scrollbar py-0.5">
          <div className="flex items-center gap-1 p-1 bg-slate-100 dark:bg-white/5 border border-slate-200/80 dark:border-white/8 rounded-xl w-max min-w-0">
            {statusFilterTabs.map(({ key, label }) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-black whitespace-nowrap transition-all duration-200 cursor-pointer ${
                    statusFilter === key
                      ? "bg-white dark:bg-[#0d1f17] text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 shadow-sm"
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full lg:flex-1 min-w-0">
          <div className="relative flex-1 min-w-0">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-[#08120e] border border-slate-200/80 dark:border-white/8 rounded-xl text-sm font-medium text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-white/25 focus:outline-none focus:border-emerald-500/50 transition-all"
            />
          </div>

          <Link
            href="/member/tugas/riwayat"
            className="group flex items-center justify-center gap-1.5 px-3.5 py-2.5 bg-white dark:bg-[#08120e] border border-slate-200/80 dark:border-white/8 hover:border-emerald-500/30 rounded-xl transition-all duration-300 hover:shadow-[0_0_16px_rgba(16,185,129,0.1)] whitespace-nowrap shrink-0"
          >
      ...
      <div className="space-y-10">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-amber-500/15">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-lg bg-amber-500/10 border border-amber-500/25 text-amber-500 dark:text-amber-400 flex items-center justify-center shadow-sm">
            ...
            {filteredMainQuests.length > 0 ? (
              ...
              <div className="py-10 bg-white dark:bg-[#08120e] border border-dashed border-amber-500/20 rounded-xl text-center text-xs text-slate-500 dark:text-white/40 flex flex-col items-center justify-center gap-2">
        ...
        {(questCategory === "ALL" || questCategory === "SIDE") && (
          <div className="space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-emerald-500/15">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 flex items-center justify-center shadow-sm">
            ...
            {filteredSideQuests.length > 0 ? (
              ...
              <div className="py-10 bg-white dark:bg-[#08120e] border border-dashed border-emerald-500/20 rounded-xl text-center text-xs text-slate-500 dark:text-white/40 flex flex-col items-center justify-center gap-2">
        ...
        {filteredMainQuests.length === 0 && filteredSideQuests.length === 0 && (
          <EmptyState
            ...
            className="py-20 bg-white dark:bg-[#08120e] border border-slate-200/80 dark:border-white/5 rounded-xl"
          />
        )}
      </div>
